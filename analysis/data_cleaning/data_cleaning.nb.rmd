...
```{r, message = FALSE, warning = FALSE, eval = TRUE, echo = TRUE, include = TRUE}
set.seed(8888)
...
```{sql, connection = db, output.var = "table_col_names"}
select t.*,c.*,k.*
...
#### Questions
  + `completed`: completed_date? What is its format?
  + `topic`: topic_id? Why all of its meta key values are 0?
  + `lesson`: we need to do text mining (121 lessons in total, sounds right?) to get lesson title.
  + `quiz`: quiz_id? Its values are from `pro_quizid`, why we have these two different variables? And what are the differences between these two?
  + `time`: activity_time? What is its format?
...
#### Questions
  + post_content: essay answers? How can I tell this answer is from which user?
  + posy_title: essay questions?
  + What is `sfwd-zapier`?
  + Why 68% of the post content of `sfwd-quiz` are missing? (same questions for course, lesson and qui_pro_id)

### postmeta table
  + What is `meta_value` in postmeta table? like course_id?
...
#### Questions
  ...
  + Are `quiz_id` the same as `quiz_pro_id`? (so that I know if it is appropriate to join quiz and quiz question tables using quiz_id.)
  + What are cloze_answer, assessment_answer, free_answer, matrix_sort_answer and sort_answer?
...
#### Questions
  + What is `create_time`
  + It has users info: form_data (need to use text mining to extract information)
...
```{r, message = FALSE, warning = FALSE, eval = FALSE, echo = FALSE, include = TRUE}
r_rdb = dbConnect(MySQL(), user = 'therefu0_steve', password = 'en1){~G?BnPv', 
                  dbname = 'therefu0_wp527', host = 'c28673.sgvps.net', port = 3306)
{
  r_users <- dbGetQuery(r_rdb, "SELECT * FROM wpt1_users")
  r_usermeta <- dbGetQuery(r_rdb, "SELECT * FROM wpt1_usermeta")
  r_activity <- dbGetQuery(r_rdb, "SELECT * FROM wpt1_learndash_user_activity")
  r_activitymeta <- dbGetQuery(r_rdb, "SELECT * FROM wpt1_learndash_user_activity_meta")
  r_posts <- dbGetQuery(r_rdb, "SELECT ID, post_title, post_type FROM wpt1_posts")
  r_postmeta <- dbGetQuery(r_rdb, "SELECT * FROM wpt1_postmeta WHERE meta_key IN ('course_id', 'lesson_id', 'question_id' 'quiz_id', 'quiz_pro_id')")
  r_quiz_question <- dbGetQuery(r_rdb, "SELECT * FROM wpt1_wp_pro_quiz_question")
  r_quiz_statistic <- dbGetQuery(r_rdb, "SELECT * FROM wpt1_wp_pro_quiz_statistic")
  r_quiz_statistic_ref <- dbGetQuery(r_rdb, "SELECT * FROM wpt1_wp_pro_quiz_statistic_ref")
  r_quiz_toplist <- dbGetQuery(r_rdb, "SELECT * FROM wpt1_wp_pro_quiz_toplist") # only for quiz 18 and quiz 169
}
...
p6 <- ggplot(test_df, aes(x =course_id)) +
  ...
  ggtitle("Number of Quiz Answered by Question Type and Course") +
  theme_bw()
p6
```
